<?php
require_once('db.php'); // Подключаем базу данных

// Получаем фильтры
$price_min = isset($_POST['price_min']) ? (int)$_POST['price_min'] : null;  // Если цена не указана, ставим null
$price_max = isset($_POST['price_max']) ? (int)$_POST['price_max'] : null;  // Если цена не указана, ставим null
$brands = isset($_POST['brand']) ? $_POST['brand'] : [];
$types = isset($_POST['type']) ? $_POST['type'] : [];
$genders = isset($_POST['gender']) ? $_POST['gender'] : [];
$age_groups = isset($_POST['age_group']) ? $_POST['age_group'] : [];
$gears = isset($_POST['gears']) ? $_POST['gears'] : [];
$search = isset($_POST['search']) ? $_POST['search'] : ''; // Получаем значение для поиска

// Подготовка SQL-запроса с фильтрами
$query = "SELECT * FROM products WHERE 1"; // Начинаем запрос с условия "1", чтобы все фильтры можно было добавить динамически
$params = [];

// Условие для цены
if ($price_min !== null) {
    $query .= " AND price >= ?";
    $params[] = $price_min;
}

if ($price_max !== null) {
    $query .= " AND price <= ?";
    $params[] = $price_max;
}

if ($brands) {
    $query .= " AND brand IN (" . implode(',', array_fill(0, count($brands), '?')) . ")";
    $params = array_merge($params, $brands);
}

if ($types) {
    $query .= " AND type IN (" . implode(',', array_fill(0, count($types), '?')) . ")";
    $params = array_merge($params, $types);
}

if ($genders) {
    $query .= " AND gender IN (" . implode(',', array_fill(0, count($genders), '?')) . ")";
    $params = array_merge($params, $genders);
}

if ($age_groups) {
    $query .= " AND age_group IN (" . implode(',', array_fill(0, count($age_groups), '?')) . ")";
    $params = array_merge($params, $age_groups);
}

if ($gears) {
    $query .= " AND gears IN (" . implode(',', array_fill(0, count($gears), '?')) . ")";
    $params = array_merge($params, $gears);
}

// Добавляем поиск по имени товара, если строка поиска не пуста
if ($search) {
    $query .= " AND name LIKE ? COLLATE UTF8_GENERAL_CI";  // Добавляем COLLATE для нечувствительности к регистру
    $searchTerm = "%" . $search . "%";  // Поиск по части имени
    $params[] = $searchTerm;  // Добавляем параметр в массив параметров
}

// Выполнение запроса
$stmt = $conn->prepare($query);
$stmt->bind_param(str_repeat('s', count($params)), ...$params); // Привязываем параметры
$stmt->execute();
$result = $stmt->get_result();

// Вывод результатов
$html = '';
while ($item = $result->fetch_assoc()) {
    // Получаем изображение для товара
    $imgQuery = "SELECT image_url FROM product_images WHERE product_id = ? AND is_main = 1";
    $stmtImg = $conn->prepare($imgQuery);
    $stmtImg->bind_param("i", $item['id']);
    $stmtImg->execute();
    $imageResult = $stmtImg->get_result();
    $image = $imageResult->fetch_assoc();

    if ($image) {
        $imageData = base64_encode($image['image_url']);
        $imageUrl = "data:image/jpeg;base64,{$imageData}";
    } else {
        $imageUrl = '/images/products/default.jpg';
    }

    // Добавляем товар в вывод
    $html .= "
    <div class='catalog-cell' data-product-id='{$item['id']}'>
        <img src='{$imageUrl}' alt='item-pic'>
        <h1>{$item['name']}</h1>
        <div class='catalog-cell-basket'>
            <p>{$item['price']} ₽</p>
            <div class='catalog-cell-basket-btn' data-product-id='{$item['id']}' data-quantity='1'>
                <svg width='20' height='20' viewBox='0 0 20 20' fill='none' xmlns='http://www.w3.org/2000/svg'>
                    <path
                        d='M16 16C14.89 16 14 16.89 14 18C14 18.5304 14.2107 19.0391 14.5858 19.4142C14.9609 19.7893 15.4696 20 16 20C16.5304 20 17.0391 19.7893 17.4142 19.4142C17.7893 19.0391 18 18.5304 18 18C18 17.4696 17.7893 16.9609 17.4142 16.5858C17.0391 16.2107 16.5304 16 16 16ZM0 0V2H2L5.6 9.59L4.24 12.04C4.09 12.32 4 12.65 4 13C4 13.5304 4.21071 14.0391 4.58579 14.4142C4.96086 14.7893 5.46957 15 6 15H18V13H6.42C6.3537 13 6.29011 12.9737 6.24322 12.9268C6.19634 12.8799 6.17 12.8163 6.17 12.75C6.17 12.7 6.18 12.66 6.2 12.63L7.1 11H14.55C15.3 11 15.96 10.58 16.3 9.97L19.88 3.5C19.95 3.34 20 3.17 20 3C20 2.73478 19.8946 2.48043 19.7071 2.29289C19.5196 2.10536 19.2652 2 19 2H4.21L3.27 0M6 16C4.89 16 4 16.89 4 18C4 18.5304 4.21071 19.0391 4.58579 19.4142C4.96086 19.7893 5.46957 20 6 20C6.53043 20 7.03914 19.7893 7.41421 19.4142C7.78929 19.0391 8 18.5304 8 18C8 17.4696 7.78929 16.9609 7.41421 16.5858C7.03914 16.2107 6.53043 16 6 16Z'
                        fill='none' />
                </svg>
            </div>
        </div>
    </div>
    ";
}

echo $html;
?>
